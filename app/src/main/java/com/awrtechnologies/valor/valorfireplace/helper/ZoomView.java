...
public class ZoomView extends FrameLayout {
    ...
    private TouchImageView touchedView;
    private TouchImageView addonImage;
    ...
    public void setFireplaceImageView(TouchImageView fireplaceImageView) {
        this.fireplaceImageView = fireplaceImageView;
    }

    public TouchImageView getFireplaceImageView() {
        return fireplaceImageView;
    }

    public void setAddonImage(TouchImageView addonImage) {
    ...
    public boolean dispatchTouchEvent(final MotionEvent ev) {
        ...
        if (ev.getActionMasked() == MotionEvent.ACTION_DOWN || ev.getAction() == MotionEvent.ACTION_POINTER_DOWN) {
            int x = Math.round(ev.getX());
            int y = Math.round(ev.getY());
            float[] f = new float[9];
            m.getValues(f);
            if (fireplaceImageView.inViewInBounds(x, y)) {
                touchedView = fireplaceImageView;
            } else {
                touchedView = null;
            }
        }
        ...
        if (touchedView != null) {
//            addonImage.onTouch(addonImage,ev);
            touchedView.onTouch(touchedView, ev);
            return true;
        } else {
            // single touch
